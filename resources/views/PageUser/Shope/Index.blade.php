<x-app-layout>
    ...
    @if (session('error'))
        <div class="max-w-4xl mx-auto mt-5 mb-4 p-4 bg-green-100 border border-red-500 text-green-700 rounded-lg shadow-md flex items-center">
    ...
    <div class="lg:grid lg:grid-cols-3 gap-4">
        <!-- Product Grid -->
        <div class="col-span-3 p-4 mt-4">
            <h2 class="text-2xl font-bold mb-4 text-center">Product</h2>
            <div id="product-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                @foreach ($products as $product)
                    <div class="w-full bg-orange-100 rounded-lg shadow-lg overflow-hidden border border-gray-300 {{ 'kue-' . strtolower($product->category->category_name ?? 'no-category') }}">
                        ...
                        <div class="px-5 pb-5 mt-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                @if ($hasDiscount)
                                    <div>
                                        <span class="text-lg font-bold text-red-500">Rp{{ number_format($discountedPrice, 2) }}</span>
                                        <span class="line-through text-gray-500">Rp{{ number_format($product->price, 2) }}</span>
                                        <span class="text-sm text-green-500">-{{ $product->discount->percentage }}%</span>
                                    </div>
                                @else
                                    <span class="text-2xl font-bold text-gray-900">Rp{{ number_format($product->price, 3, ',', '.') }}</span>
                                @endif
                            ...
                            <div class="flex items-center mt-2 mb-4">
                                <span class="ml-1 text-sm font-semibold text-gray-900">Stok: {{ $product->stok_quantity }}</span>
                            </div>
                            <a href="#">
